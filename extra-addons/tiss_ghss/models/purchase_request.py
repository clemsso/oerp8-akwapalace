...
class PurchaseRequest(models.Model):
    ...
    def button_to_approve(self):
        ...
        filter = [('budget_id','=',self.budget_id.id),('b_validate','=',True)]
...
class PurchaseRequestLine(models.Model):
    ...
    budget_post_id = fields.Many2one('account.budget.post',
                                     related='request_id.budget_post_id',
                                     string='Budget Head', store=True, readonly=True)
    spendplan_id = fields.Many2one('budget.spendplan', related='request_id.spendplan_id', string="Spend Plan")
    find_selection = fields.Selection(related='request_id.find_selection', string="Budget Head", store=True, readonly=True)
    
    sp_supplier_activities_id = fields.Many2one('spendplan.supplier.activities', string="Supplier Act.", 
    related='request_id.sp_supplier_activities_id', store=True, readonly=True)
    sp_fringe_activities_id = fields.Many2one('spendplan.fringe.activities', string="Fringe Act.",
    related='request_id.sp_fringe_activities_id', store=True, readonly=True)
    sp_travel_activities_id = fields.Many2one('spendplan.travel.activities', string="Travel Act.",
    related='request_id.sp_travel_activities_id', store=True, readonly=True)
    sp_other_activities_id = fields.Many2one('spendplan.other.activities', string="Others Act.",
    related='request_id.sp_other_activities_id', store=True, readonly=True)
    ...
    budget_id = fields.Many2one('crossovered.budget',
                                related='request_id.budget_id',
                                string='Budget', store=True, 
                                readonly=True)
    ...
    def onchange_sp_travel_id(self):
        if self.sp_travel_id:
            currency_amount=self.spendplan_id.budget_id.currency_amount
            self.planned_amount = self.sp_travel_id.total_price*currency_amount

    @api.onchange('sp_fringe_id')        
    def onchange_sp_fringe_id(self):
        if self.sp_fringe_id:
            currency_amount=self.spendplan_id.budget_id.currency_amount
            self.planned_amount = self.sp_fringe_id.annual_pay*currency_amount

    @api.onchange('sp_supplier_id')        
    def onchange_sp_supplier_id(self):
        if self.sp_supplier_id:
            currency_amount=self.spendplan_id.budget_id.currency_amount
            self.planned_amount = self.sp_supplier_id.amount_subtotal*currency_amount
