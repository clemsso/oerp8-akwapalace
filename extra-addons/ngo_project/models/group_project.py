...
#				self.allowance_ids = [(4,[allowance_id.id])]
			...
			for slip in self.payslip_batch_id.slip_ids:
				if slip.contract_id in self.contract_ids:
					self.total_gross += slip.gross_salary
					self.total_taxable += slip.taxable_salary
					self.total_pit += slip.pit
					self.total_act += slip.act
					self.total_ctax += slip.ctax
					self.total_cfce += slip.cfce
					self.total_cfcp += slip.cfcp
					self.total_fne += slip.fne
					self.total_crtv += slip.crtv
					self.total_net += slip.net
					self.total_basic += slip.basic
					self.total_cnpse += slip.cnpse
					self.total_cnpsp += slip.cnpsp
					self.total_ipa += slip.ipa
					self.total_fcon += slip.fcon
					self.total_other_deductions += slip.extra_deduction
...
class HR_Payslip_Batch_run(models.Model):
	...
	def generate_dipe(self):
        ...
        # contractJob_obj = self.env['hr.contract.job']
        # projetContrat_obj = self.env['project.contract']
        ...
        if self.slip_ids:
            for payslip in self.slip_ids:
                for contract in payslip.contract_id.contract_job_ids:
                    ...
                    #
                    # val={'project_history_id':project_history_id.id,
                         # 'percentage':contract.percentage,
                         # 'employee_id':payslip.contract_id.employee_id.id,
                         # 'contract_id':payslip.contract_id.id,
                        # }
                    # projetContrat_id=projetContrat_obj.create(val)
                    
		# if self.slip_ids:
			# for payslip in self.slip_ids:
				# if payslip.contract_id.project_template_id:
					# project_history_id = history_obj.search([('project_id','=',payslip.contract_id.project_template_id.id),('payslip_batch_id','=',self.id)])
					
					# if not project_history_id:
						# name = "%s - %s" % (payslip.contract_id.project_template_id.name,self.name)
						# project_id = payslip.contract_id.project_template_id.id
						# project_history_id = history_obj.create({'name': name, 'project_id': project_id, 'payslip_batch_id':payslip_run_id})	
					# project_history_id.contract_ids = [(4, payslip.contract_id.id)]
