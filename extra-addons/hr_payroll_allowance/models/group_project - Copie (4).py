...
class HR_Payroll_History(models.Model):
	...
	amt_all_history_id = fields.One2many('hr.contract.project.history.all', 'history_id', string='Gross')
	...
	@api.one
	def get_gross(self):
		for l in self.amt_gross_history_id:
			l.unlink()
	...
	def _update_payroll_component_project(self):
		...
		if self.payslip_batch_id.slip_ids:
            ...
            # for allowance in self.allowance_ids:
				...
				if allowance.allowance_id.id in all 
				line_number += 1
