...
#		return nb_days
	...
	represent = fields.Float(string="Represent Allowance",compute="_get_amount",readonly=True,digits_compute=dp.get_precision('Payroll'))
	risk = fields.Float(string="Risk Allowance",compute="_get_amount",readonly=True,digits_compute=dp.get_precision('Payroll'))
	...
	def _get_amount(self):
		...
		represent = self.line_ids.search([('code','=','REPRESENT'),('slip_id','=',self.id)])
		risk = self.line_ids.search([('code','=','RISK'),('slip_id','=',self.id)])
		
		if represent:
			self.represent = represent[0].total
		else:
			self.represent = 0
		
		if risk:
			self.risk = risk[0].total
		else:
			self.risk = 0
