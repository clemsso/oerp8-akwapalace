...
class HR_Payroll_History(models.Model):
	...
	# @api.one
	# def get_gross(self):
		# for b in self.amt_basic_history_id:
			# b.unlink()
		# for g in self.amt_gross_history_id:
			# g.unlink()
		# for a in self.amt_all_history_id:
			# a.unlink()
		# if self.contract_ids:
			# for contract in self.contract_ids:
				# perc=100.0
				# for p in contract.contract_job_ids:
					# if p.project_id.id == self.project_id.id:
						# if len(contract.contract_job_ids)>1:
						    # perc = p.percentage
						# valb = {'contract_id':contract.id, 'amt_basic':p.wage*0.4, 'history_id':self.id}
						# basic_id=self.env['hr.contract.project.history.basic'].create(valb)
						# val = {'contract_id':contract.id, 'amt_gross':p.wage, 'history_id':self.id}
						# gross_id=self.env['hr.contract.project.history.gros'].create(val)
                        # for all in contract.allowance_ids:
							# vals = {'contract_id':contract.id, 'allowance_id':all.set_id.id, 'history_id':self.id, 'amt_global':all.local_amount, 'percentage':perc, 'amt':all.local_amount*perc/100.0}
							# all_id=self.env['hr.contract.project.history.all'].create(vals) 
		# return True
	# @api.one
	# def get_gross(self):
		# print"---get_gross---"
		...
		# if self.contract_ids:
		    # for payslip in self.payslip_batch_id.slip_ids:
		        # if payslip.contract_id in self.contract_ids:
		            # perc=100.0
		            # if len(payslip.contract_id.contract_job_ids)>1:
		            ...
		            # for line in payslip.line_ids:
		                # if line.salary_rule_id.code == 'BASIC':
		                    # amt_basic = line.total*perc/100.00
		                    # valb = {'contract_id':payslip.contract_id.id, 'amt_basic':amt_basic, 'history_id':self.id}
		                    # basic_id=self.env['hr.contract.project.history.basic'].create(valb)         
		                # if line.salary_rule_id.code == 'GROSS':
		                    # amt_gross = line.total*perc/100.00
		                    # val = {'contract_id':payslip.contract_id.id, 'amt_gross':amt_gross, 'history_id':self.id}
		                    # gross_id=self.env['hr.contract.project.history.gros'].create(val)
		# return True
	@api.one
	def get_gross(self):
			...
			for slip in self.payslip_batch_id.slip_ids:
				if slip.contract_id in self.contract_ids:
					...
					for contract in slip.contract_id.contract_job_ids:
						if self.project_id.id == contract.project_id.id:
							if len(slip.contract_id.contract_job_ids)>1:
								perc = contract.percentage
